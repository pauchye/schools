...
class Compare extends React.Component {
  constructor(props) {
    super(props)
    this.state = { onlyDifferences: false }
  }

  render() {
    const { schools, compareDbns, removeFromCompare } = this.props
    const compared = compareDbns.map((dbn) => schools.find((s) => s.dbn === dbn)).filter(Boolean)
      ...
      <div className="compare-page">
        <div className="compare-header">
          ...
          <div className="compare-header-controls">
            ...
            <label className="compare-diff-toggle">
              <input type="checkbox" checked={this.state.onlyDifferences}
                onChange={(e) => this.setState({ onlyDifferences: e.target.checked })} />
        ...
        {(() => {
          ...
          rows.forEach((row) => {
            ...
            const known = rawValues.filter((v) => v !== null)
            ...
            if (this.state.onlyDifferences && allSame) return
